...
software = {
    'imod':     { 'version': '4.9.11',   'module': 'imod/4.9.11' },
    'eman2':    { 'version': '20190603', 'module': 'eman2/20190603' },
    #'ctffind4': { 'version': '4.1.13',  'module': 'ctffind/4.1.13' },
    'ctffind4': { 'version': '4.1.10',  'module': 'ctffind4-4.1.10-intel-17.0.4-rhn26cm' },
    #'motioncor2': { 'version': '1.2.2', 'module': 'motioncor2/1.2.2' },
    'motioncor2': { 'version': '1.1.0', 'module': 'motioncor2-1.1.0-gcc-4.8.5-zhoi3ww' },
    'relion':   { 'version': '3.0.4',   'module': 'relion/3.0.4' }
}
...
yes
100
{% if 'phase_plate' in dag_run.conf and dag_run.conf['phase_plate'] == '1' %}yes
0
1.571
0.1
{%- else %}no{% endif %}
...
{% endif %}
    ...
    motioncorr_stack = LSFSubmitOperator( task_id='motioncorr_stack',
        ...
        retry_delay=timedelta(seconds=1),
        lsf_script="""#!/bin/bash -l
#BSUB -o {{ dag_run.conf['directory'] }}/aligned/motioncor2/{{ params.software.motioncor2.version }}/{{ dag_run.conf['base'] }}_aligned.job
{% if params.convert_gainref %}#BSUB -w "done({{ ti.xcom_pull( task_ids='convert_gainref' )['jobid'] }})"{% endif %}
...
#BSUB -W {% if superres %}9{% else %}7{% endif %}
...
""",
    ...
    drift_data = MotionCor2DataSensor( task_id='drift_data',
        filepath="{{ dag_run.conf['directory'] }}/aligned/motioncor2/{{ params.software.motioncor2.version }}/{{ dag_run.conf['base'] }}_aligned.log0-*Full.log",
        poke_interval=5,
        timeout=30,
        params={
         "software": software,
    ...
    convert_aligned_preview = LSFOperator( task_id='convert_aligned_preview',
        ...
        timeout=300,
        lsf_script="""#!/bin/bash -l
#BSUB -o {{ dag_run.conf['directory'] }}/aligned/motioncor2/{{ params.software.motioncor2.version }}/{{ dag_run.conf['base'] }}_aligned_preview.job
...
""",
        params={
            'software': software,
            'apix': args['apix'] if 'apix' in args else None,
            'particle_size': args['particle_size'] if 'particle_size' in args else 150,
        }
    )
    ...
    previews = BashOperator( task_id='previews',
        ...
        bash_command="""
            ...
            convert \
                ...
                +append -font DejaVu-Sans -pointsize 28 -fill SeaGreen1 -draw 'text 8,492 \"~{{ ti.xcom_pull( task_ids='particle_pick_data' ) }} pp\"' \
                +append -font DejaVu-Sans -pointsize 28 -fill yellow -draw 'text 520,492 "'${timestamp}'"' \
                +append -font DejaVu-Sans -pointsize 28 -fill yellow -draw 'text 854,492 \"{{ '%0.1f' | format(ti.xcom_pull( task_ids='summed_ctf_data' )['resolution']) }}Å ({{ '%d' | format(ti.xcom_pull( task_ids='summed_ctf_data' )['resolution_performance'] * 100) }}%)\"' \
            ...
            convert \
                ...
                +append  \
                -font DejaVu-Sans -pointsize 28 -fill orange -draw 'text 854,46 \"{{ '%0.1f' | format(ti.xcom_pull( task_ids='aligned_ctf_data' )['resolution']) }}Å ({{ '%d' | format(ti.xcom_pull( task_ids='aligned_ctf_data' )['resolution_performance'] * 100) }}%)\"' \
...
echo 'clearing old value'
              ...
              echo "{{ ti.xcom_pull( task_ids='aligned_file' )[0].replace( dag_run.conf['directory'], '' ) }}  {{ ti.xcom_pull( task_ids='aligned_ctf_file' )[0].replace( dag_run.conf['directory'], '' ) }}:mrc   {{ ti.xcom_pull( task_ids='aligned_ctf_data' )['defocus_1']  }}  {{ ti.xcom_pull( task_ids='aligned_ctf_data' )['defocus_2']  }}  {{ ti.xcom_pull( task_ids='aligned_ctf_data' )['cs']  }}  {{ ti.xcom_pull( task_ids='aligned_ctf_data' )['additional_phase_shift']  }}  {{ ti.xcom_pull( task_ids='aligned_ctf_data', key='context' )['acceleration_voltage']  }}  {{ ti.xcom_pull( task_ids='aligned_ctf_data', key='context' )['spherical_aberration'] }}  {{ ti.xcom_pull( task_ids='aligned_ctf_data', key='context' )['amplitude_contrast']  }}  10000  {{ ti.xcom_pull( task_ids='aligned_ctf_data', key='context' )['pixel_size']  }}  {{ ti.xcom_pull( task_ids='aligned_ctf_data' )['cross_correlation']  }}  {{ ti.xcom_pull( task_ids='aligned_ctf_data' )['resolution']  }}" >> $STAR_FILE
            } 3<>$STAR_FILE
